...
TestAndLearn.Home.Grids = (function () {
    ...
    return {
        init: function (config) {
            ...
            this.initClosedGrid();
            this.initNotApprovedGrid();
            this.initAllTestsGrid();
            ...
            $('#test-tab-panel a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                ...
                if (e.target.toString().indexOf('submitted') > -1 ) {
                    ...
                    $('#download-tests-form').trigger('reset');     //to clear all filtering options
                    $('#submitted-test-grid input[id*="gs_"]').val("");

                    //this.clearFilters();
                    $('#submitted-test-grid').setGridParam({ datatype: 'json' });
                    $('#submitted-test-grid').trigger('reloadGrid');
                }
                else if (e.target.toString().indexOf('approved') > -1 && e.target.toString().indexOf('notapproved') <= -1) {
                    $('#approved-test-grid').setGridWidth($('#approved').width());

                    $('#download-tests-form').trigger('reset');     
                    $('#approved-test-grid input[id*="gs_"]').val("");
                    ...
                    $('#download-tests-form').trigger('reset');     
                    $('#closed-test-grid input[id*="gs_"]').val("");
                ...
                else if (e.target.toString().indexOf('notapproved') > -1) {
                    $('#notapproved-test-grid').setGridWidth($('#notapproved').width());

                    $('#download-tests-form').trigger('reset');     
                    $('#notapproved-test-grid input[id*="gs_"]').val("");

                    var colModel = $('#notapproved-test-grid').jqGrid('getGridParam', 'colModel');
                    $('#notapproved-test-grid').setColWidth('Name', colModel[4].width - 18, false);
                    $('#notapproved-test-grid').jqGrid('setGridParam', { datatype: 'json' }).trigger('reloadGrid');
                }
                else if (e.target.toString().indexOf('alltests') > -1) {
                    $('#all-test-grid').setGridWidth($('#alltests').width());

                    $('#download-tests-form').trigger('reset');     
                    $('#all-test-grid input[id*="gs_"]').val("");

                    var colModel = $('#all-test-grid').jqGrid('getGridParam', 'colModel');
                    $('#all-test-grid').setColWidth('Name', colModel[4].width - 18, false);
                    $('#all-test-grid').jqGrid('setGridParam', { datatype: 'json' }).trigger('reloadGrid');
                }
            ...
            $('#download-tests-form').submit(function (e) {
                ...
                if (f != null) {
                    for (var i = 0; i < f.rules.length; i++) {
                        ...
                        if (fieldName == "Pillar.Name") {
                            $(this).find('input[name="PillarQuery"]').val(f.rules[i].data);
                            ...
                            JQueryUX.PopUpMenu.show(e, {
                                ...
                                items: [
                                      ...
                                      },
                                      {
                                          name: 'Park Test',
                                          onClick: function () {
                                              TestAndLearn.Home.Grids.changeTestStatus(id, _config.notapprovedStatusId);
                                          }
                                      },
                                      {
                                          name: 'Add Urls',
        ...
        , initNotApprovedGrid: function () {
            ...
            $('#notapproved-test-grid').jqGrid({
                url: _config.getTestsUrl,       //get parked tests
                postData: {
                    clientId: _config.clientId,
                    testStatusId: _config.notapprovedStatusId     //status id of parked
                },
                datatype: "json",
                mtype: 'GET',
                autowidth: true,
                ...
                loadComplete: function (data) {
                    ...
                    if (data.length == 0 || (typeof (data.rows) != 'undefined' && data.rows.length == 0)) {
                        ...
                        $('#notapproved-test-grid .options-cell').unbind('click');
                        $('#notapproved-test-grid .options-cell').click(function (e) {
                            var id = $(this).attr('id').replace('options-cell-', '');
                        ...
                        $('#notapproved-test-grid .delete-test').unbind('click');

                        $('#notapproved-test-grid .delete-test').click(function (e) {
                            var id = $(this).attr('id').replace('delete-test-row-', '');
                            TestAndLearn.Home.DeleteTestForm.deleteTest(id);
            ...
            $('#notapproved-test-grid').jqGrid('filterToolbar', {
                stringResult: false,
                searchOnEnter: false,
                defaultSearch: 'cn'
            });
        ...
        , initAllTestsGrid: function () {
            ...
            $('#all-test-grid').jqGrid({
                url: _config.getTestsUrl,
                postData: {
                    clientId: _config.clientId,
                    //testStatusId: _config.approvedStatusId     //get all Tests 
                },
                datatype: "json",
                mtype: 'GET',
                autowidth: true,
                ...
                loadComplete: function (data) {
                    ...
                    if (data.length == 0 || (typeof (data.rows) != 'undefined' && data.rows.length == 0)) {
                        ...
                        //$('#all-test-grid .options-cell').unbind('click');

                        //$('#all-test-grid .options-cell').click(function (e) {
                        //    var id = $(this).attr('id').replace('options-cell-', '');
                        ...
                        $('#all-test-grid .delete-test').unbind('click');

                        $('#all-test-grid .delete-test').click(function (e) {
                            var id = $(this).attr('id').replace('delete-test-row-', '');
                            TestAndLearn.Home.DeleteTestForm.deleteTest(id);
            ...
            $('#all-test-grid').jqGrid('filterToolbar', {
                stringResult: false,
                searchOnEnter: false,
                defaultSearch: 'cn'
            });
